	...
	public function send($addToQueue = false) {
     ...
     //       $mailer->AddAddress($email);
			...
			if ($addToQueue) {
			    ...
			    $status = $mailer->send_mail($toEmailInfo, $fromEmail, $fromEmail, $subject, $mailer->Body, $cc, $bcc);
			//	$status = $mailer->SendDirect($mailer,false, $this->get('parent_id'));
			} else {
			    //	$status = $mailer->Send(true);
			    $status = $mailer->send_mail($toEmailInfo, $fromEmail, $fromEmail, $subject, $mailer->Body, $cc, $bcc);
			//	$status = $mailer->SendDirect($mailer,false);
			}
			if(!$status) {
